...
e=input("Enter the full url:-")
temp=e

def fetch(temp):
    try:
        response = requests.get(temp, verify=False, headers=random_headers(), timeout=10)
        return response
...
def union(str):
    ...
    for aas in dict.keys():
        if aas in a:
            str=str.replace(aas,"Pwned by eDG36 w4f")
            ss.append(dict[aas])
    ...
    if count>=1:
        for aas in dict.keys():
            if aas in a:
                totals=totals.replace(aas,"Pwned by eDG36 w4f")
                ss.append(dict[aas])
